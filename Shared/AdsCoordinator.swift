...
private let AdPresentationHeightInTiles = 3
private let ShowEveryXLine = 5
...
class AdsCoordinator {
    ...
    func removeAdLines(_ lines: Int) -> Int {
        let adSectionHeight = ShowEveryXLine + AdPresentationHeightInTiles
        let numberOfSections = Int(lines / adSectionHeight)
        
        if numberOfSections == 0 {
            return lines
        }
        
        let reminder = lines % (numberOfSections * adSectionHeight)
